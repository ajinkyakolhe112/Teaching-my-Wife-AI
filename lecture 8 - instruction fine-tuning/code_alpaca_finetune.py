...
TRAINING_CONFIG = {
    ...
    "max_length": 256,  # Increased for code examples
    "num_train_epochs": 1,
    "batch_size": 4,
    "learning_rate": 2e-4,
}
...
def train_model(trainer):
    ...
    trainer.train()
    print("Saving model...")
    trainer.save_model("./code_alpaca_model_final")

# Example inference function
def generate_code(model, tokenizer, device, instruction, input_text=""):
    prompt = f"""
    ...
    outputs = model.generate(
        **inputs,
        max_length=200,  # Increased for code generation
...
if __name__ == "__main__":
    ...
    print(f"Output: {generate_code(model, tokenizer, device, test_instruction, test_input)}") 